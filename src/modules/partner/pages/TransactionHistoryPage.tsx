...
export function TransactionHistoryPage() {
  ...
  useEffect(() => {
    void vipApi.listPurchases('partner').then(setPurchases)
  }, [])
  ...
  return (
    <PartnerLayout nav={MANAGEMENT_NAV}>
      ...
      <p className="mb-6 text-sm text-brand-ink-soft">
        Toàn bộ lượt mua/gia hạn Skola VIP của công ty. Hệ thống chưa có cổng thanh toán thật nên mọi giao dịch được ghi nhận "đã thanh toán" ngay khi nâng cấp.
      </p>
      ...
      {purchases === null ? (
        ...
        <EmptyState
          ...
          description="Nâng cấp Skola VIP để hồ sơ học bổng của công ty được ưu tiên hiển thị."
          action={
            <Link to="/skola-vip?tab=partner">
            ...
            <TableBody>
              {purchases.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="font-medium text-brand-ink">✨ Skola VIP · Doanh nghiệp</TableCell>
